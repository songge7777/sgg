...
## 2、设备独立像素 && css像素 && dpr
- 设备独立像素，可以认为是计算机坐标系统中的一个点，这个点代表一个可以由程序使用并控制的虚拟像素。
...
### 移动端自适应
- `npm install -S webpack@5.74.0 webpack-cli@4.10.0 css-loader@6.7.1 px2rem-loader@0.1.9 style-loader@3.3.1 html-webpack-plugin@5.5.0 webpack-dev-server@4.11.0 lib-flexible`
...
<img :src="$withBase('/img/wxmin1.png')" >

<img :src="$withBase('/img/wxmin2.png')" >
...
```html
<view class="layout">
  <image class="img" src="../../static/mylove.jpg"></image>
  <text class="name">我爱颖宝</text>
...
```html
<view class="layout">
  <image class="img" src="{{userInfo.avatarUrl ? userInfo.avatarUrl:img}}"></image>
  <text class="name">{{userInfo.nickName ? userInfo.nickName: name}}</text>
  <button wx:if="{{!userInfo.nickName}}" bindtap="getUserInfo">获取用户信息</button>
  <text wx:else class="welcome">{{welcome}}</text>
</view>
```
## 4、路由跳转
